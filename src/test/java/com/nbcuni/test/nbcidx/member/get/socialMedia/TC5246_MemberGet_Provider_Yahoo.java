...
/**************************************************************************
 * NBCIDX Member.Get API - TC5246_MemberGet_Provider_Yahoo. Copyright. 
 * 
 * @author Rashmi Sale
 * @version 1.0 Date: July 4, 2014
 **************************************************************************/

public class TC5246_MemberGet_Provider_Yahoo {
	...
	public static void memberGet_providerYahoo() throws Exception {
		
		//Find the yahoo member from MongoDB
		DBObject DbObj = member.findRandomUUIDWithSocialInfo(mydb, "yahoo");
        ...
        Reporter.log("Fetched UUID from MongoDB for Yahoo member = "+finalUUID);
		...
		JsonElement provider = api.getJsonArray(metadata, "_provider").get(0).getAsJsonObject().get("value").getAsJsonObject().get("yahoo");
        sProviderId=provider.toString();
        finalProviderId=sProviderId.substring(1, sProviderId.length()-1);
        Reporter.log("Fetched corresponding Yahoo provider id = " +finalProviderId );
        ...
        Reporter.log("2) Verifying member.get API Response with id=<yahoo id> & provider=yahoo ");
        String params = "id="+finalProviderId+"&provider=yahoo";
        ...
        Reporter.log("3) Verifying member.get with id=<UUID> & provider=yahoo ");
        params = "id="+finalUUID+"&provider=yahoo";
        ...
        if(bUsername)
        {
            Reporter.log("4) Verifying member.get with id=<username> & provider=yahoo ");
            params = "id="+sUsername+"&provider=yahoo";
        ...
        if(bEmail)
        {
            Reporter.log("Verifying member.get with id=<Email> & provider=yahoo ");
            params = "id="+finalEmail+"&provider=yahoo";
