...
/**************************************************************************
 * NBCIDX Member.Get API - TC5247_MemberGet_Provider_Linkedin. Copyright. 
 * 
 * @author Rashmi Sale
 * @version 1.0 Date: July 4, 2014
 **************************************************************************/

public class TC5247_MemberGet_Provider_Linkedin {
	...
	public static void memberGet_providerLinkedin() throws Exception {
		
		//Find the linkedin member from MongoDB
		DBObject DbObj = member.findRandomUUIDWithSocialInfo(mydb, "linkedin");
        ...
        Reporter.log("Fetched UUID from MongoDB for Linkedin member = "+finalUUID);
		...
		JsonElement provider = api.getJsonArray(metadata, "_provider").get(0).getAsJsonObject().get("value").getAsJsonObject().get("linkedin");
        sProviderId=provider.toString();
        finalProviderId=sProviderId.substring(1, sProviderId.length()-1);
        Reporter.log("Fetched corresponding Linkedin provider id = " +finalProviderId );
	    ...
	    Reporter.log("2) Verifying member.get API Response with id=<linkedin id> & provider=linkedin ");
        String params = "id="+finalProviderId+"&provider=linkedin";
		...
		Reporter.log("3) Verifying member.get with id=<UUID> & provider=linkedin ");
        params = "id="+finalUUID+"&provider=linkedin";
        ...
        if(bUsername)
        {
            Reporter.log("4) Verifying member.get with id=<username> & provider=linkedin ");
            params = "id="+sUsername+"&provider=linkedin";
        ...
        if(bEmail)
        {           
            Reporter.log("5) Verifying member.get with id=<Email> & provider=linkedin ");
            params = "id="+finalEmail+"&provider=linkedin";
