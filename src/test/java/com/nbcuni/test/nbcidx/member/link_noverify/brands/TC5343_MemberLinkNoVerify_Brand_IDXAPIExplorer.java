...
/***********************************************************************************************
 * NBCIDX Member.Link_noverify API - TC5343_MemberLinkNoVerify_Brand_IDXAPIExplorer. Copyright. 
 * 
 * @author Rashmi Sale
 * @version 1.0 Date: Sep 10, 2014
 ***********************************************************************************************/

public class TC5343_MemberLinkNoVerify_Brand_IDXAPIExplorer {
	...
	@Test(groups = {"full"})
	public void memberLinkNoVerify_IDXAPIExplorer() throws Exception {
		
		Reporter.log("Validating member.link_noverify with brand = IDX API Explorer");
		Reporter.log(" ");
		...
		int mylink = ma.memberLINKNoVerifyResponseCode(api, al, postBody, idxAPIExplBrandId);	
		if (mylink ==200)
		...
		mylink = ma.memberLINKNoVerifyResponseCode(api, al, postBody, idxAPIExplBrandId);	
		if (mylink ==200)
		...
		Reporter.log("5) Removing member from brand = IDX Explorer");
		//Generate Post Body.
		postBody = "id="+uuid;

		//Send member.remove POST Request 
		int removeReturnCode = ma.memberREMOVEResponseCode(api, al, postBody, idxAPIExplBrandId, username, uuid, mydb);
