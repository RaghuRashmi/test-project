...
/************************************************************************************
 * NBCIDX Member.Link_noverify API - TC5354_MemberLinkNoVerify_Error_400. Copyright. 
 * 
 * @author Rashmi Sale
 * @version 1.0 Date: Sep 10, 2014
 ************************************************************************************/

public class TC5354_MemberLinkNoVerify_Error_400 {
	...
	@Test(groups = {"full"})
	public void memberLinkNoVerify_error400() throws Exception {
		...
		Reporter.log("3) Member.link_noverify with wrong param 'i'");
		...
		Reporter.log("4) Member.link_noverify with no provider");
		myJsonBody ="id="+uuid+"&=facebook&provider_id=1002470365";
		mylink = ma.memberLINKNoVerifyResponseCode(api, al, myJsonBody, surfBrandId);
		if(mylink == 400)
			Reporter.log("Passed : Member.link_noverify Response code ="+mylink);
		else
			fail("Member.link_noverify Response code ="+mylink);
		...
		Reporter.log("5) Member.link_noverify with no params");
		myJsonBody ="";
		mylink = ma.memberLINKNoVerifyResponseCode(api, al, myJsonBody, surfBrandId);
		if(mylink == 400)
			Reporter.log("Passed : Member.link_noverify Response code ="+mylink);
		...
		Reporter.log("6) Member.link_noverify with no provider_id");
		...
		Reporter.log("7) Member.link_noverify with only uuid");
		myJsonBody ="id="+uuid;
		mylink = ma.memberLINKNoVerifyResponseCode(api, al, myJsonBody, surfBrandId);
		if(mylink == 400)
			Reporter.log("Passed : Member.link_noverify Response code ="+mylink);
