...
/**************************************************************************************
 * NBCIDX Member.Link_noverify API - TC5339_MemberLinkNoVerify_username. Copyright. 
 * 
 * @author Rashmi Sale
 * @version 1.0 Date: Sep 10, 2014
 **************************************************************************************/

public class TC5339_MemberLinkNoVerify_username {
	...
	@Test(groups = {"full"})
	public void memberLinkNoVerify_username() throws Exception {
		...
		Reporter.log("");
		Reporter.log("2) Calling member.link_noverify with username, provider = testProvider and providerID = "+providerID);
		Reporter.log("");
		
		// Generate Post body to link with username.
		String postBody ="id="+username+"&provider=testProvider&provider_id="+providerID;
		int mylink = ma.memberLINKNoVerifyResponseCode(api, al, postBody, surfBrandId);	
		if (mylink ==200)
			Reporter.log("Linking username with link_noverify is successful");
		else
