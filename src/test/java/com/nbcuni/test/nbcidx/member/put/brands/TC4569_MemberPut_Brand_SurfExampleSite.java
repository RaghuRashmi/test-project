...
/*****************************************************************************
 * NBCIDX Member.Put API - TC4930_MemberRemove_Brand_SurfExampleSite. Copyright. 
 * 
 * @author Rashmi Sale
 * @version 1.0 Date: July 4, 2014
 *****************************************************************************/

public class TC4569_MemberPut_Brand_SurfExampleSite {
     ...
     */
	@Test(groups = {"full"})
	public void memberPut_SurfExampleSite() throws Exception {
				
		Reporter.log("1) Validating member.put with brand = Surf Example Site");
		...
		jsonBody = "";
		jsonBody ="{'_id': '"+sApiUUID+"','suffix': 'MS','phone': [{'mobile': true,'number': '8884444333'}],'prefix': 'Dr.','brand_data': {'MyChoice': 'Surf Example Site'},'birthdate': {'year': 2013,'day': 23,'month': 7},'avatar': 'http://www.nbc.com/','screenname': 'Master'}";
		...
		bMemberBrandData = brandDataArrlst.get(0).toString().contains("MyChoice:Surf Example Site");
		Reporter.log(brandDataArrlst.get(0).toString());
		Assert.assertEquals(bMemberBrandData, true);
		...
		Reporter.log("7) Removing the member from IDX");
