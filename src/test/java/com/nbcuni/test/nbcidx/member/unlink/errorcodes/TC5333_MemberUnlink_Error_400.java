...
/**************************************************************************
 * NBCIDX Member.unlink API - TC5333_MemberUnlink_Error_400. Copyright. 
 * 
 * @author Rashmi Sale
 * @version 1.0 Date: Sep 10, 2014
 **************************************************************************/

public class TC5333_MemberUnlink_Error_400 {
	...
	@Test(groups = {"full"})
	public void memberUnlink_error400() throws Exception {
		...
		Reporter.log("3) Member.unlink with wrong param 'i'");
		...
		Reporter.log("4) Member.unlink with blank 'provider'");
		myJsonBody ="id="+uuid+"&provider=&provider_id=1002470365";
		mylink = ma.memberUNLINKResponseCode(api, al, myJsonBody, surfBrandId);
		if(mylink == 400)
			Reporter.log("Passed : Member.unlink Response code ="+mylink);
		else
			fail("Member.unlink Response code ="+mylink);
		...
		Reporter.log("5) Member.unlink with no provider");
		...
		Reporter.log("6) Member.unlink with no params");
		myJsonBody ="";
		mylink = ma.memberUNLINKResponseCode(api, al, myJsonBody, surfBrandId);
		if(mylink == 400)
			Reporter.log("Passed : Member.unlink Response code ="+mylink);
		...
		Reporter.log("7) Member.unlink with no provider_id");
		...
		Reporter.log("8) Member.unlink with only uuid");
		myJsonBody ="id="+uuid;
		mylink = ma.memberUNLINKResponseCode(api, al, myJsonBody, surfBrandId);
		if(mylink == 400)
			Reporter.log("Passed : Member.unlink Response code ="+mylink);
		...
		Reporter.log("9) Member.unlink with blank gigya_api_key & site_id");
		...
		Reporter.log("10) Member.unlink with blank site_id");
